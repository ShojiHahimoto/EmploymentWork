...
モーション編集機能は一度に全機能を追加せず、次の段階を順番に実装する。各段階が完了したら、既存の数値入力、キーフレーム保存、プレビュー再生、バトル中のモーション再生が壊れていないことを確認してから次へ進む。
...
- JSON Loader / Saver は現在すでにボーン位置の項目を扱えるが、編集画面の主な編集対象は回転である
- Common Motion には見た目だけの `rootOffsetKeys` があり、AttackData には対戦中の実座標を動かす `movementKeys` がある
- MotionSystem はボーンのローカル姿勢を親子階層へ反映する FK 方式で、キーフレーム間の補間とモーション遷移ブレンドを行う
- 可動域制限、プレビュー上のクリック選択、ギズモ、IK は未実装である

##### 位置データと回転データの責務

編集時に関節を平行移動できるようにすることと、モーション結果として各ボーンの位置を毎フレーム保存することは別の問題として扱う。通常の人型モデルではボーン間の長さと bind pose のローカル位置は ModelResource が保持する固定情報であり、モーションでは主にローカル回転を変化させる。

- `ModelResource::ModelBone::bindLocalPosition` は骨格の基準位置として保持し、モーション編集で書き換えない
- 新しいモーション編集では、通常の関節移動を MotionData の毎フレーム位置キーとして保存しない
- 編集画面で手や足などを動かす位置は、IK の目標位置または編集時だけの操作用データとして扱う
- IK が計算した結果は、最初の実装では各ボーンのローカル回転へ変換して通常の FK キーフレームとして保存する
- 現在存在する `hasPosition / localPosition` は既存 JSON との互換性を確認するまで削除しない。ただし新しい編集 UI からは原則として書き込まず、旧データを読み込める受け皿として残す
- 全身の見た目だけを上下前後へずらす Common Motion の `rootOffsetKeys` と、攻撃中にキャラクター本体・判定・カメラへ反映する AttackData の `movementKeys` は、ボーン姿勢の位置編集と混同しない
- 攻撃の踏み込みや突進など対戦上の移動は必ず AttackData の移動として扱い、モーション姿勢の位置キーで代用しない

位置と回転を同時に無制限で保存すると、親子階層、骨の長さ、当たり判定の基準が競合しやすい。そのため、位置を動かした結果を IK / FK で回転へ変換し、実行時には既存の FK 再生だけを使う方針とする。

##### FK と IK の編集上の役割

- FK は親から子へローカル姿勢を伝える方式とする。親部位を回転・移動した場合は、その子孫が親子階層に従って追従する
- FK で子部位を直接編集した場合、親部位を自動的に動かして目標位置へ合わせることはしない
- IK は手や足などの終端位置を指定し、親側の関節角度を自動計算する編集補助として扱う
- IK は現段階では対戦中の処理に追加しない。CustomizeScene のプレビューと編集時だけ使用する
- 最初の IK は 2 ボーンチェーンに限定する。腕は肩-肘-手、脚は股関節-膝-足を対象とする
- IK の計算結果はローカル回転へ焼き込み、バトル中は従来の MotionSystem / MotionData の FK 再生だけで動かす
- 肩を動かしたときに背中や腰まで追従させる機能は、腕・脚の 2 ボーン IK とは別の階層制約または全身 IK として後段で追加する
- IK の目標位置を後から再編集できる必要が生じた場合だけ、MotionData の実行用姿勢と混ぜず、任意の `editorData` または別 JSON に IK 目標・ポール方向を保存する
- FK 編集と IK 編集を同じフレームで行った場合は、最後に実行した編集を正とする。IK を再適用した場合は、そのチェーンの FK 回転を上書きする

##### モーション編集の段階番号

この12段階をモーション編集機能の唯一の実装順序として扱う。以前の説明や会話内の別番号ではなく、この一覧を正とする。

- ユーザーが「次の段階の実装に進んでください」または同じ意味の合図を出した場合、直前に完了した段階の次の番号へ進む
- 合図の文言が完全一致しなくても、次段階へ進む意図が明確なら同じ運用とする
- 追加で必要な基盤作業や不具合修正がある場合は、段階の順序を壊さない範囲で先に行う
- 追加作業によって次段階の内容が変わる場合は、実装前に変更理由を説明し、このメモへ反映する
- 現在は第1〜第6段階実装済み（実画面確認待ち）。第6段階の動作確認後、次の合図では第7段階「プリセットの外部データ化」へ進む。未確認の不具合があれば先に第6段階を修正する

1. 部位定義の共通化
   - 15 部位の正式名称、モデル側のボーン名候補、親部位、左右関係を 1 か所へ集約する
   - `CustomizeScene` と `MotionSystem` が同じ定義を使い、プルダウン、クリック選択、可動域、IK の部位対応を一致させる
   - 正式名称は `Head`, `Spine`, `Waist`, `RShoulder`, `LShoulder`, `RElbow`, `LElbow`, `RHand`, `LHand`, `RHipjoint`, `LHipjoint`, `RKnees`, `LKnees`, `RFeet`, `LFeet` とする
   - 完了済み。`MotionSkeletonDefinition` に共通定義を追加し、既存の重複定義を置き換えた

2. 姿勢計算部分の整理
   - bind pose、現在姿勢、親子階層反映、ローカル姿勢とワールド姿勢の変換を編集用処理として整理する
   - `CustomizeScene` が `MotionSystem` の内部処理へ直接依存しすぎないよう、編集用の一時姿勢を扱う境界を作る
   - 親を操作した場合に子孫が追従し、子を操作しても親が勝手に変化しない FK の動作を確認する
   - 編集用姿勢と対戦中の `SkeletonPoseComponent` を混同しない。編集結果は既存 MotionData へ反映する
   - この段階ではギズモ、クリック選択、IK、可動域制限を追加しない
   - 2026-09-10 実装完了。`Data/SkeletonPose.h` に Component から独立した姿勢データを置き、`System/MotionPose` に bind pose、補間、FK、座標変換を集約した
   - `CustomizeScene` は独立した `SkeletonPose` を保持し、対戦側は `SkeletonPoseComponent` を保持する。双方とも `MotionPose` を呼び、再生時計・ActionState・遷移ブレンドは引き続き `MotionSystem` が管理する
   - FK は15部位の論理親子関係ではなく、補助ボーンを含む ModelResource の実親階層を使用する。既存名 `boneWorldMatrices` の実体はモデル空間行列であり、本体 Transform を掛けて初めて実ワールド行列になる
   - 編集結果は従来どおり draftMotion のキーへ即時反映する。全身キー単位の編集、JSON の保存形式、攻撃の実座標移動、汎用モーションの見た目オフセットは変更していない
   - `tools/tests/RunMotionPoseTests.ps1` で親から子孫への追従、子編集時の親不変、実ワールドとローカルの往復変換、逆変換不能時の非更新、補間・ループを確認済み。Debug|x64 ビルド成功。実画面での編集・保存再読込の目視確認は別途必要

3. 本体ウィンドウへのプレビュー移行
   - 本体ウィンドウ左側を 3D プレビュー、右側を ImGui 編集領域として使う
   - 専用ビューポート、シザー矩形、リサイズ対応、描画後の Renderer 状態復元を実装する
   - 現在の RenderTexture 経路は新経路が安定するまで残してよい。移行確認後に不要部分を削除する
   - 2026-09-10 実装。MotionEditor と技パラメータ編集は本体左側のバックバッファへ直接描画し、右側の編集欄と左下の再生操作欄をメイン ImGui viewport に固定する。旧 RenderTexture 経路は新経路が安定するまで補助として残す
   - カスタマイズプレビューの背景色は `Renderer::GetDefaultClearColor()` を使い、通常ゲーム画面の既定背景と揃える。今後ステージ・背景モデルを追加する場合も、この直接描画経路へ通常の3D背景描画を足す
   - アタック編集とモーション編集の左プレビュー・右編集欄レイアウト計算は共通化する。`CustomizeScene` の本格分割は、次段階以降も既存挙動を確認しながら小さく進める
   - `Renderer::ScopedRenderRegion` が viewport、scissor、出力先、深度・ブレンド・ラスタライザー状態、ビュー・投影行列を退避・復元する。深度クリアは全体へ作用するため、本体プレビューは当該シーン最初の3Dパスとして使う
   - リサイズ後のクライアント寸法から毎描画時に矩形を求め、そのアスペクト比をプレビューカメラへ渡す。技編集へ戻る場合は RenderTexture のアスペクト比へ戻す。編集値変更後にプレビューを描画するため、同フレームに反映される
   - Debug|x64 ビルド確認済み。実行環境からゲームウィンドウを取得できなかったため、モデル表示、再生・停止・コマ送り、カメラ操作、リサイズ、技編集との往復、タイトルへの復帰は Visual Studio 実行で確認が必要。関節マーカー・ギズモ・IK は未実装のまま

4. 関節マーカーとクリック選択
   - 各関節のワールド座標をプレビュー画面へ投影し、一定のピクセル範囲内で最も近い関節を選択する
...
5. 数値による FK 編集
   ...
   - 内部は Quaternion、編集 UI は Euler 角とする
   - 親を編集した場合は子孫も追従させ、子を編集しても親は自動変更しない
   - 既存の数値入力と即時プレビュー反映を維持する
   - 位置操作はこの段階では毎フレームのボーン位置キーとして新規保存しない
   - 2026-09-12 整理完了。既存の数値回転編集を正式な FK 編集基盤として扱う。キーフレームがあるフレームだけ編集可能、クリック選択とプルダウン選択は同じ選択部位へ同期、キー追加直後も同フレームで数値編集できる。選択部位番号は必ず有効範囲へ補正し、不正部位による編集破綻を防ぐ。親子追従は `MotionPose` の実ボーン階層 FK に委託し、編集 UI 側で親子を個別に動かす処理は持たない

6. 可動域制限
   - 部位ごとの回転可動域と編集用の移動範囲を共通部位定義またはモデルプロファイルに持たせる
   - 可動域は技ごとに保存せず、同じモデルの同じ関節で共有する
   - 数値入力、ギズモ操作、IK 計算結果、JSON 読み込み時の検証へ同じ制限を適用する
   - 角度の折り返し、クランプ、親子変換を一貫して処理する
   - 2026-09-15 実装。`MotionSkeletonDefinition` の共通部位定義に `MotionJointRotationLimit` を追加し、数値入力と MotionData キー書き込み時に同じ制限を通す。制限値はモーションの誇張を許すため現実より甘めにする。`Waist` は現状の全身ルート回転として使うため可動域制限なしとし、人体の腰関節としては扱わない。人体的な胴体制限は `Spine` から適用する
   - 左右対称ポーズを同じ数値で作りやすくするため、左側部位には編集UI表示用の回転符号を持たせる。これは MotionData の内部保存値を変換するものではなく、表示時に内部値から編集値へ、入力保存時に編集値から内部値へ戻すだけの変換とする。既存モーションJSONの内部値と再生結果は維持する

7. プリセットの外部データ化
   - T ポーズなどをコードへ直書きせず、プリセット JSON として管理できるようにする
   - 全身プリセットは 15 部位へ適用し、選択部位プリセットは現在選択中の部位だけへ適用する
   - プリセット適用先は現在フレームにキーフレームがある場合だけとし、編集方針に反して勝手にキーを作らない
   - 2026-09-15 実装。姿勢プリセットは `assets/PosePreset/*.json` に保存し、保存時は常に 15 部位の全身姿勢を保存する。読み込み時はプリセット選択後に適用部位を複数選択し、選択部位だけを現在フレームの既存キーフレームへ適用する
   - モーション編集画面に常時表示する入口は `Save Pose Preset` と `Load Pose Preset` の 2 つとする。保存名は空欄不可、重複不可、ファイル名として危険な文字を禁止する
   - `T Pose` はコード直書きではなく `assets/PosePreset/TPose.json` として扱う。`Idle Pose` は保存プリセットではなく `Common/Idle` の 0F 姿勢を読み込み時に参照する特殊プリセットとして扱う
   - 2026-09-16 修正。日本語名を Windows ファイル名へ直接使うと文字化けや `std::filesystem` 例外の原因になるため、PosePreset のファイル名は `pose_0001.json` のような ASCII 安全 ID に固定し、日本語の名前は JSON 内の `displayName` として保存する。重複判定もファイル名ではなく `displayName` で行う
   - 現在は第1〜第7段階実装済み。第7段階の動作確認後、次の合図では第8段階「ギズモの回転編集」へ進む。未確認の不具合があれば先に第7段階を修正する

8. ギズモの回転編集
   - まず回転、選択中部位、ローカル座標操作に限定する
   - ギズモで操作したワールド行列は、親ボーンの逆行列を使って選択部位のローカル姿勢へ戻して保存する
   - ギズモ操作結果にも数値入力と同じ可動域制限を適用する
   - ギズモは自作より既存の ImGui 対応ライブラリの利用を優先し、導入できない場合のみ小さな操作範囲で自作する

9. 編集専用 IK
   - 腕と脚の 2 ボーン IK、エンドエフェクタ、ポール方向、関節制限を追加する
   - 最初は IK 結果を FK 回転へ焼き込む方式とし、実行時の新しい IK 更新処理は作らない
   - モデルごとの骨の向き、左右反転、到達不能な目標位置、肘・膝の反転を検証する

10. ギズモの移動編集
   - 手・足の移動は IK の目標位置を移動する操作として扱う
   - IK の計算結果をローカル回転へ変換し、実行用 MotionData へ FK キーとして保存する
   - 攻撃中の実座標移動は `AttackData::movementKeys` のままとし、ギズモの見た目操作と混ぜない

11. 肩・背中・腰の階層制約
   - 肩から背中、腕から肩など、複数階層へ影響する制約を必要に応じて追加する

12. 保存と編集補助
   - IK 目標の再編集保存が必要になった場合だけ、実行用姿勢とは分離した編集用 IK データを追加する
   - Undo / Redo、左右ミラー、姿勢リセット、角度スナップはこの段階以降に追加する

##### 各段階の確認項目

- 既存のモーション JSON が読み込める
- 保存して再読み込みしても姿勢が変わらない
- キーフレームのないフレームでは編集できない
- 親を動かしても子ボーンの階層が壊れない
- DebugPlayer と DebugPlayer2 の両方で同じ編集用部位が解決できる
- モーションの補間、ループ、遷移ブレンドに影響がない
- 攻撃の実移動、ヒットボックス、カメラ位置がモーション編集の見た目操作によって変化しない
- IK の編集機能を追加しても、バトル中の処理順と 60fps 固定のルールを変更しない
