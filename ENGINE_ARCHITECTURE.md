...
StateUpdateSystem は、Player タグと Transform / InputHistory / State を持つ GameObject を対象にする。
現段階では InputHistoryComponent のテンキー方向と攻撃ボタンを見て、`Idle`、`Walk`、`GroundAttack`、`AirAttack`、`Fall`、`Hitstun` を含む `PlayerActionState` を確定する。
...
現段階では確定済み `PlayerActionState::Walk` のときだけ、InputHistoryComponent のテンキー方向を使って Velocity の X 成分を毎フレーム上書きする。
Velocity はノックバック、技移動、押し出しでも変化するため、Idle / Walk 判定には使わない。
Jump / Fall は接地やジャンプ処理が未実装のため、勝手に Idle / Walk へ戻さず維持する。
...
- 攻撃、ジャンプ、ガードなどのボタンは InputSystem が判定済みの `Trigger / Press / Release` を保存する
- 現段階では今フレーム分だけを保存する
